...
export default {
  ...
  // Ничего не изменяет, занимается вычислением? -- Переносим в computed
  // Computed -- про семантику, делаем не для производительности хоть бонусом и получаем
  computed: {
    startIndex() {
  ...
  methods: {
    ...
    async updateTickers() {
      ...
      //   ticker.price = price ?? "-";
      // });
      // if (this.selectedTicker?.name === this.tickers.name) {
  ...
  watch: {
    ...
    // когда выбирается тикер -- сбрось график
    selectedTicker() {
      this.graph = [];
    },
